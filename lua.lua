...
lua.table = lua.ast('table', lua.ignore(parser_exact('{')), parser_transform(parser_sep(lua.field, parser_exact(',')), astlist), lua.ignore(parser_exact('}')))
...
lua.pre = parser_first(lua.ast('length', lua.ignore(parser_exact('#')), lua.post), lua.ast('negate', lua.ignore(parser_exact('-')), lua.post), lua.ast('not', lua.keyword('not'), lua.post), lua.post)
...
local function syntaxstr(ast, vars)
    ...
    elseif ast.type == 'or' then
        return '[lua.or(lua.first(' .. syntaxstr(ast[1], vars) .. '),()=>lua.first('.. syntaxstr(ast[2], vars) .. '))]'
    elseif ast.type == 'and' then
        return '[lua.and(lua.first(' .. syntaxstr(ast[1], vars) .. '),()=>lua.first('.. syntaxstr(ast[2], vars) .. '))]'
    elseif ast.type == 'table' then
        local fun = {}
        fun[#fun+1] = '(()=>{var n=0,t=new Map();'
        for i=1, #ast do
            local field = ast[i]
    ...
    elseif ast.type == 'for' then
        ...
        return 'for (let i=' .. inrange[1] .. ',m=' .. inrange[2] .. ',c=' .. inrange[3] .. ';i<=m;i+=c){var ' .. start .. '=i;' .. body .. '}'
